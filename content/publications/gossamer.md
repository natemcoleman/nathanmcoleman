---
title: "Modeling And Analysis Of Inter-Panel Slipping For The Design Of Rolled Gossamer Arrays"
description: "Published in the Journal of Mechanisms and Robotics IDETC24 Special Issue and the International Design Engineering Technical Conferences & Computers and Information in Engineering Conference 2024"
# date: January 2025
draft: false
weight: 400
tags: ["matlab", "optimization"]
cover:
    image: "publications/gossamer/RollModelSimpleExample.jpg"
---

For more details, you can visit the publication page on the ASME Digital Collection:
<a href="https://asmedigitalcollection.asme.org/mechanismsrobotics/article-abstract/17/4/044505/1207337/Modeling-and-Analysis-of-Inter-Panel-Slipping-for?redirectedFrom=fulltext" target="_blank">View Publication</a>

You can view and download the PDF of the publication using the link below:
<a href="/publications/gossamer/IDETC_Rolled_Gossamer_Slip_Modeling.pdf" target="_blank">Download PDF</a>

You can view and download the PDF of the Journal of Mechanisms and Robotics publication using the link below:
<a href="/publications/gossamer/FMANU-JMR-24-1460.pdf" target="_blank">Download PDF</a>

For the source code and additional materials, you can visit the GitHub repository:
<a href="https://github.com/natemcoleman/RolledMembraneMatlab" target="_blank">GitHub Repository</a>



<div style="display: flex; justify-content: center; flex-wrap: wrap; gap: 20px;">
    <img src="/publications/gossamer/HeatMapMaxSlippage/BendRadiusVNumPanels.jpg" alt="Photo 1" style="width: 45%;">
    <img src="/publications/gossamer/HeatMapMaxSlippage/ThicknessVBendRadius.jpg" alt="Photo 2" style="width: 45%;">
    <img src="/publications/gossamer/HeatMapMaxSlippage/ThicknessVNumPanels.jpg" alt="Photo 3" style="width: 45%;">
    <img src="/publications/gossamer/HeatMapMaxSlippage/TotalLengthVBendRadius.jpg" alt="Photo 4" style="width: 45%;">
    <img src="/publications/gossamer/HeatMapMaxSlippage/TotalLengthVNumPanels.jpg" alt="Photo 5" style="width: 45%;">
    <img src="/publications/gossamer/HeatMapMaxSlippage/TotalLengthVThickness.jpg" alt="Photo 6" style="width: 45%;">
</div>
